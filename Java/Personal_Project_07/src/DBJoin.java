...
public class DBJoin implements MouseListener {
    ...
    Statement stmt = null;
    ResultSet rs = null;
    String url = "jdbc:oracle:thin:@192.168.219.126:1521:xe"; // 오라클 포트번호1521/@이후에는 IP주소
    String sql = null;
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        try {
            ...
            if (e.getSource().equals(checkBt)) {
                sql = "select * from userinfo where id='" + idTf.getText() + "'";
            ...
            if (e.getSource().equals(joinBtn)) {
                ...
                } else if ((idTf.getText().isEmpty()) == true || (pwTf.getText().isEmpty()) == true) {		// id 혹은 pw 비어있을경우
                    msgbox.messageBox(logPanel3, "비어있는 칸이 존재합니다.");
                } else {

                    sql = "insert into userinfo (id, password) values ('" + idTf.getText() + "','" + pwTf.getText() + "')";
