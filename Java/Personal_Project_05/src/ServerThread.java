...
public class ServerThread implements Runnable {
    ...
    private void addBadword(String word){
        ...
        try{
            thisUser.writeUTF(User.ALARM + "/금칙어 등록 완료");
        }catch (IOException e){
            e.printStackTrace();
        }
                    ...
                    if(user.getAdmin() == true){                // 나가는 유저가 admin이면
                        for(int j = 0; j < roomArray.get(i).getMonitorArray().size(); j++){
                            if(user.getId().equals(roomArray.get(i).getMonitorArray().get(j).getId())){
                                ...
                                // 사용자의 방리스트에서 방을 제거
                                for (int k = 0; j < user.getRoomArray().size(); k++) {
                                    if (Integer.parseInt(rNum) == user.getRoomArray().get(k).getRoomNum()) {
                                        user.getRoomArray().remove(k);
                                    }
                                }
