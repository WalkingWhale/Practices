    ...
    public synchronized void dataParsing(String data) {
        ...
        switch (protocol) {
            ...
            case User.ECHO01: // 대기실 에코
                msg = token.nextToken();
                msgToken = new StringTokenizer(msg, " ");
                msgCensored = censor(msgToken);
                echoMsg(msgCensored);
                break;
...
//                    wait(1000);
    ...
    private String censor(StringTokenizer msg){
        ...
        while(msg.hasMoreTokens()){
            temp = msg.nextToken();
            for(int i = 0; i < this.getUser().getPersonalBadword().size(); i++){
                if(temp.equals(this.getUser().getPersonalBadword().get(i))){
                    temp = "****";
                }
            }
            newMsg += temp + " ";
